...
n = np.linspace(0.1, 5, 300) # generate 100 n values in n=[0,10]
c = np.linspace(0.1, 5, 300) # generate 100 c values in c=[0,10]
...
im = ax2.contour3D(N, C, U, levels=45, cmap="jet")
...
ax3.set_xticks(np.arange(0,5.5,0.5), labels=np.arange(0,5.5,0.5), fontsize=9,rotation=65)
ax3.set_yticks(np.arange(0,5,1))
ax3.set_xlabel("N")
ax3.set_ylabel("C")

ax4 = fig.add_subplot(224)
ax4.contour(N, C, U, levels=45, cmap="jet")
plt.colorbar(im, ax=ax4, cmap="jet")
ax4.set_xticks(np.arange(0,5.5,0.5), labels=np.arange(0,5.5,0.5), fontsize=9,rotation=65)
